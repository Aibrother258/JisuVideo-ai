...
## 评审处理（CHANGES_REQUESTED → 收口）

Codex 对 PR #13 的评审（`Aibrother258`）导航实现复核通过、63 项测试复跑通过，要求合入前完成两项文档收口：

1. **文档状态对齐事实**：`docs/ui-optimization-plan.md` 头部 v1.0（初稿待评审）/「尚未进入实施」与第 8 章「PR #11 未合入」均已过期 → 升版 **v1.2**：头部改为「P0-F 已实施、随 PR #13 待合入，P1–P4 仍待排期」；修订表补 v1.2 行；第 8 章同步 PR #11 已合入 master（`437026b`）、P0-F 按其建议时序落地无冲突。
2. **文档合入路径去重**：PR #12 与本 PR 同带一份 `docs/ui-optimization-plan.md` → 采纳评审建议路径一：**由本 PR 保留方案文档（v1.2）与 P0-F 实现，PR #12 已标记被取代并关闭**，避免双份合并。
3. **非阻断项一并落地**：① 导航按钮补 `aria-current="page"`（当前目录），配套结构测试断言；② `?tab=` URL 深链在方案中明确为**已延期独立增强项**（4.3 表改标注 + 第 10 章新增 D7），不再随页面拆分模糊待议。
