...
| HB-20260904-04 | UI C4 暗色主题首批（B1）——dark token 覆盖块 + `data-theme` 切换 + 首帧 FOUC + B1 字面量 token 化 | 完成（PR #53，merge `7c99e15`）；经三轮评审收口（静态 head bootstrap / 真构建产物集成测试 + CI 强制 / 子进程兜底清理）；frontend 132/132、test:build 通过 | [查看归档](../pr-records/2026-09-04-ui-c4-dark-theme-b1.md) |
