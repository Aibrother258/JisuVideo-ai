...
> 说明：**主线二「多视频类型扩展」整体取消**，本文不再实施、不再据此排期，保留仅供追溯参考；后续主规划以 `docs/product-positioning-roadmap.md` 为准（定位与路线主文档，2026-09-04 起；UI 治理专项自 v3.4 进入维护状态，见 `docs/ui-optimization-plan.md`；修订记录 v3.3 登记本取消决策）。shuohao 五件套导入方向保留为「短剧上游导入」候选方向（待单独立项），与本稿第 7 章轨道脱钩。  
