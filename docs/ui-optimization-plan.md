...
| 在途事项 | 与本文重叠面 | 建议 |
| --- | --- | --- |
| 「多视频类型扩展完整实施方案」v2.2 冻结稿（对应 `docs/multi-video-type-extension-plan` 分支） | R5/R6 将改造创建流程（`index.vue`）与建集入口，新增视频类型会波及工作台与详情页字段/流程 | P0-F（设置页）与 P1（Token 收敛）无重叠可先行；**B2 顺序经 v2.8 决策更新（见修订记录 v2.8）并已执行：script 面板纯搬迁拆分（PR #47）先行启动并已合入（merge `58c1e96`，2026-09-04），其余面板拆分等扩展前端改造合入后再实施**，避免双向 rebase 冲突 |
