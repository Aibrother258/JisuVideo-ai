...
| 6 | （可选）URL 深链 | 用 `useRoute`/`useRouter` 同步 `?tab=styles`，刷新/分享保持所在二级目录 |
...
| 近期活跃改动：`settings.vue` Agent/Skills 数据源（commit `2f7a343`、`3a79d7c`，当前 `feat/style-ai-expansion` 分支未合入） | 与 P0-F 同在 `settings.vue` | F 专项只动导航区 + tab 数据结构，不碰 Agent/Skills 数据逻辑；建议等上述提交合入 `master` 后再实施 F，冲突面最小 |
