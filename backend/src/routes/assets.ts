...
app.get('/', async (c) => {
  ...
  // 已提供但非合法正整数 → 400。非法值绝不静默省略过滤
  // （否则会把受限查询放大成全量，造成跨项目数据误展示）。
  ...
  const episodeIdRes = parseRecordId(c.req.query('episode_id'))
  if (episodeIdRes.kind === 'invalid') return badRequest(c, 'episode_id 参数必须是合法正整数')
  const episodeId = episodeIdRes.kind === 'id' ? episodeIdRes.id : undefined
